...
function saveTasks() {
  const taskList = document.getElementById('listadd');
  const spans = taskList.querySelectorAll('.span-1');
  const tasks = [];

  spans.forEach((span, index) => {
    tasks.push({ [`Task ${index + 1}`]: span.textContent });
  });
...
function LoadTask() {
  ...
  // Task will be only editable when the page gets loaded
